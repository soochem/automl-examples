...
def add_fit_args(parser):
    ...
    train.add_argument('--macrobatch-size', type=int, default=0,
                       help='dist-learning effective batch size')
    train.add_argument('--warmup-epochs', type=int, default=5,
                       help='the epochs to ramp-up lr to scaled large-batch value')
    train.add_argument('--warmup-strategy', type=str, default='linear',
                       help='the ramping-up strategy for large batch sgd')
    train.add_argument('--profile-worker-suffix', type=str, default='',
                       help='profile workers actions into this file. During dist-learning training\
    ...
    train.add_argument('--profile-server-suffix', type=str, default='',
                       help='profile server actions into a file with name like rank1_ followed by this suffix \
                             during dist-learning training')
    train.add_argument('--use-imagenet-s-augmentation', type=int, default=0,
                       help='enable s augmentation of ImageNet s, default disabled')
    return train
...
def fit(args, network, data_loader, **kwargs):
    ...
    data_loader : function that returns the train and val s iterators
    """
    # kvstore
    ...
    if 'dist' in args.kv_store and not 'async' in args.kv_store:
        logging.info('Resizing training s to %d batches per machine', epoch_size)
